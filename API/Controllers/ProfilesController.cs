...
public class ProfilesController:BaseApiController
{
    ...
    public async Task<ActionResult> GetActivities(string userId,string filter)
    {
        return HandleResult(await Mediator.Send(new GetUserActivities.Query{UserId=userId,Filter=filter}));
    }

    [HttpPut("{userId}")]
    public async Task<ActionResult> EditProfile(string userId, string displayName, string bio)
    {
        return HandleResult(await Mediator.Send(new EditProfile.Command{UserId=userId,DisplayName=displayName,Bio=bio}));
    }
}
